...
/**
 * Provides a form for reverting a Progress entity revision.
 *
 * @ingroup d8_progress_entity
 */
class ProgressEntityRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'progress_entity_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Progress entity: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Progress entity %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\d8_progress_entity\Entity\ProgressEntityInterface $revision
   ...
   */
  protected function prepareRevertedRevision(ProgressEntityInterface $revision, FormStateInterface $form_state) {
